...
class DioxideGenerator extends GeneratorForAnnotation<dioxide.RestApi> {
  ...
  /// The global CallAdapter InterfaceType resolved from @RestApi(callAdapter:), if any.
  InterfaceType? _globalCallAdapterType;
  ...
  String _implementClass(ClassElement element, ConstantReader? annotation) {
    ...
    final serializerEnumString =
        (annotation?.peek('serializer')?.revive().accessor);
    final deserializerEnumString =
        (annotation?.peek('deserializer')?.revive().accessor);
    final serializerParser = dioxide.Parser.values
        .firstWhereOrNull((e) => e.toString() == serializerEnumString);
    final deserializerParser = dioxide.Parser.values
        .firstWhereOrNull((e) => e.toString() == deserializerEnumString);
    ...
    // Resolve global callAdapter type from @RestApi(callAdapter: SomeAdapter)
    _globalCallAdapterType =
        annotation?.peek('callAdapter')?.typeValue as InterfaceType?;
    ...
    ConstructorElement? superClassConst,
      ...
      Constructor((c) {
        ...
        if (superClassConst != null) {
          ...
          for (var element in constParams) {
          ...
          final paramList =
              constParams.map((e) => (e.isNamed ? '${e.name}: ' : '') + e.name);
          c.initializers
  ...
  Iterable<Method> parseMethods(ClassElement element) {
    return getMethodElements(element).expand((m) {
      // Per-method @UseCallAdapter takes priority over the global one.
      final methodAdapterAnnotation = getCallAdapterAnnotation(m);
      final adapterType = (methodAdapterAnnotation
              ?.peek('callAdapter')
              ?.typeValue as InterfaceType?) ??
          _globalCallAdapterType;

      // Validate: the adapter subclass must have exactly one type parameter.
      if (adapterType != null && adapterType.typeArguments.isEmpty) {
        throw InvalidGenerationSourceError(
          'CallAdapter subclass `${adapterType.element.name}` must accept a single type parameter <T>.\n'
          'Example: class MyAdapter<T> extends CallAdapter<Future<T>, Future<Result<T>>>',
          element: m,
        );
      }

      return generateMethodWithAdapter(m, _generateRequest,
          callAdapterType: adapterType);
    });
  }
  ...
  Code _generateRequest(MethodElement m, ConstantReader httpMethod) {
    ...
    final wrappedReturnType = getResponseType(m.returnType);
    ...
    final bool isWrapped =
        typeChecker(dioxide.HttpResponse).isExactlyType(wrappedReturnType);
    final returnType =
        isWrapped ? getResponseType(wrappedReturnType) : wrappedReturnType;
    if (returnType == null || 'void' == returnType.toString()) {
      ...
      if (typeChecker(Response).isExactlyType(returnType)) {
      ...
      } else if (typeChecker(List).isExactlyType(returnType) ||
          typeChecker(BuiltList).isExactlyType(returnType)) {
        if (isBasicType(innerReturnType)) {
          ...
          if (clientAnnotation.deserializer ==
              dioxide.Parser.DartSingleMapper) {
            ...
            if (clientAnnotation.compute) {
              blocks.add(refer('$_resultVar.data')
                  .conditionalIsNullIf(
                      ...
                      whenFalse: refer('await compute')
                          .call([refer('_dioxideDeserialize'), param]))
                  .assignVar('value')
                  .statement);
            } else {
              blocks.add(refer('$_resultVar.data')
                  .conditionalIsNullIf(
                      thisNullable: returnType.isNullable,
                      whenFalse:
                          refer('await _dioxideDeserialize').call([param]))
                  .assignVar('value')
                  .statement);
            }
          } else {
            ...
            switch (clientAnnotation.deserializer) {
              case dioxide.Parser.MapSerializable:
                mapperCode = refer(
                    '(dynamic i) => ${displayString(innerReturnType)}.fromMap(i as Map<String,dynamic>)');
                break;
              case dioxide.Parser.JsonSerializable:
                mapperCode = refer(
                    '(dynamic i) => ${displayString(innerReturnType)}.fromJson(i as Map<String,dynamic>)');
                break;
      ...
      } else if (typeChecker(Map).isExactlyType(returnType) ||
          typeChecker(BuiltMap).isExactlyType(returnType)) {
        ...
        if (types.length > 1) {
          ...
          if (typeChecker(List).isExactlyType(secondType) ||
              typeChecker(BuiltList).isExactlyType(secondType)) {
...
''');
        ...
        if (isBasicType(returnType)) {
          ...
          switch (clientAnnotation.deserializer) {
            case dioxide.Parser.MapSerializable:
              mapperCode = refer(
                  '${displayString(returnType)}.fromMap($_resultVar.data!)');
              break;
            case dioxide.Parser.JsonSerializable:
              ...
              if (typeArgs.isNotEmpty && genericArgumentFactories) {
                ...
                mapperCode = refer(
                    '${displayString(returnType)}.fromJson($_resultVar.data!)');
              }
              break;
            case dioxide.Parser.DartJsonMapper:
              mapperCode = refer(
                  'JsonMapper.fromMap<${displayString(returnType)}>($_resultVar.data!)!');
              break;
            case dioxide.Parser.DartSingleMapper:
              mapperCode = refer('''
              await ${clientAnnotation.compute ? 'compute(_dioxideDeserialize,' : '_dioxideDeserialize('}
      ...
      '''));
        ...
        blocks.add(
            Code('$returnAsyncWrapper value as ${displayString(returnType)};'));
      }
    }
  ...
  bool isGenericArgumentFactories(DartType? dartType) {
    ...
    if (constDartObj != null &&
        (!typeChecker(List).isExactlyType(dartType) &&
            !typeChecker(BuiltList).isExactlyType(dartType))) {
      try {
  ...
  String _getInnerJsonSerializableMapperFn(DartType dartType) {
    ...
    if (typeArgs.isNotEmpty) {
      if (typeChecker(List).isExactlyType(dartType) ||
          typeChecker(BuiltList).isExactlyType(dartType)) {
    ...
    ''';
        ...
        for (DartType arg in typeArgs) {
          ...
          if (typeArgs.isNotEmpty) {
            if (typeChecker(List).isExactlyType(arg) ||
              ...
              if (isGenericArgumentFactories(arg)) {
                ...
                mappedVal +=
                    '(json)=>${displayString(arg)}.fromJson(json as Map<String,dynamic>),';
              }
            }
          } else {
            mappedVal += _getInnerJsonSerializableMapperFn(arg);
      ...
      return refer('_setStreamType').call([
        refer('Options')
            ...
            .call(
                [],
                {}
                  ..addAll(buildBaseUrlOptions(
                      extraOptions: args, dioRef: refer(_dioVar)))
      ...
      return newOptions
          ...
          .call(
              ...
              Map.from(extraOptions)
                ...
                ..addAll(buildBaseUrlOptions(
                    extraOptions: extraOptions, dioRef: refer(_dioVar)))
                ..addAll(buildTimeoutOptions(m)))
          .cascade('data')
  ...
  Method _generateSerializeMethod() {
    return Method((m) {
      ...
      m
        ...
        ..body = Code(
            '''dynamic result = dioxideSerialize(args['dataType'] as String, args['data']);
    if (result is Future) {
  ...
  Method _generateDeserializeMethod() {
    return Method((m) {
      ...
      m
        ...
        ..modifier = MethodModifier.async
        ..body = Code(
            '''dynamic result = dioxideDeserialize(args['resultType'] as String, args['data']);
    if (result is Future) {
      return await result as T;
    } else {
      return result as T;
  ...
  void _generateRequestBody(
    ...
    if (_bodyName != null) {
      ...
      final toFormData =
          annotation.item2.peek('toFormData')?.boolValue ?? false;
      ...
      if (TypeChecker.fromRuntime(Map).isAssignableFromType(_bodyName.type)) {
        ...
        if (toFormData) {
          blocks.add(refer('FormData.fromMap($_targetVar)')
              .assignFinal(_dataVar)
              .statement);
        }
      } else if (bodyTypeElement != null &&
          ((typeChecker(List).isExactly(bodyTypeElement) ||
                  typeChecker(BuiltList).isExactly(bodyTypeElement)) &&
        ...
        if (toFormData) {
          blocks.add(refer('''FormData().addList('$_bodyName', $_targetVar)''')
              .assignFinal(_dataVar)
              .statement);
        }
      } else if (bodyTypeElement != null &&
          typeChecker(File).isExactly(bodyTypeElement)) {
        ...
        if (clientAnnotation.serializer == dioxide.Parser.MapSerializable) {
          ...
          if (toMap == null) {
            log.warning(
                '${displayString(_bodyName.type)} must provide a `toMap()` method which return a Map.\n'
            ...
            if (_bodyName.type.nullabilitySuffix !=
              ...
              blocks.add(refer('$_targetVar.addAll')
                  .call([refer('${_bodyName.displayName}.toMap()')]).statement);
            } else {
              blocks.add(refer('$_targetVar.addAll').call([
          ...
          if (_missingToJson(ele)) {
            log.warning(
                '${displayString(_bodyName.type)} must provide a `toJson()` method which return a Map.\n'
    ...
    if (parts.isNotEmpty) {
      ...
      parts.forEach((p, r) {
        ...
        if (isFileField) {
        ...
        } else if (typeChecker(List).isExactlyType(p.type) ||
            typeChecker(BuiltList).isExactlyType(p.type)) {
          ...
          if (displayString(innerType) == 'List<int>') {
            ...
            ''').statement);
              ...
              typeChecker(File).isExactlyType(innerType)) {
            final conType = contentType == null
                ? ''
                : 'contentType: MediaType.parse(${literal(contentType)}),';
            ...
            if (p.type.isNullable) {
              ...
              typeChecker(MultipartFile).isExactlyType(innerType)) {
            if (p.type.isNullable) {
              blocks.add(Code('if (${p.displayName} != null) {'));
            }
            blocks
        ...
        } else if (typeChecker(Map).isExactlyType(p.type) ||
            typeChecker(BuiltMap).isExactlyType(p.type)) {
          blocks.add(refer(_dataVar).property('fields').property('add').call([
            refer('MapEntry').newInstance(
                [literal(fieldName), refer('jsonEncode(${p.displayName})')])
          ]).statement);
    ...
    switch (clientAnnotation.serializer) {
      ...
      case dioxide.Parser.DartSingleMapper:
        return !ele.functions.any((element) =>
            element.name == 'dioxideDeserialize' &&
