...
import 'package:diopa/diopa.dart' as diopa;
...
class DiopaOptions {
  ...
  DiopaOptions({this.autoCastResponse});

  DiopaOptions.fromOptions([BuilderOptions? options])
      : autoCastResponse =
            (options?.config['auto_cast_response']?.toString() ?? 'true') ==
                'true';
}

class DiopaGenerator extends GeneratorForAnnotation<diopa.RestApi> {
  ...
  final DiopaOptions globalOptions;

  DiopaGenerator(this.globalOptions);

  /// Annotation details for [RestApi]
  late diopa.RestApi clientAnnotation;
  ...
  String _implementClass(ClassElement element, ConstantReader? annotation) {
    ...
    final parser =
        diopa.Parser.values.firstWhereOrNull((e) => e.toString() == enumString);
    clientAnnotation = diopa.RestApi(
      autoCastResponse: (annotation?.peek('autoCastResponse')?.boolValue),
      baseUrl: (annotation?.peek(_baseUrlVar)?.stringValue ?? ''),
      parser: (parser ?? diopa.Parser.JsonSerializable),
    );
  ...
  ConstantReader? _getHeadersAnnotation(MethodElement method) {
    final annotation = _typeChecker(diopa.Headers)
        .firstAnnotationOf(method, throwOnUnresolved: false);
  ...
  ConstantReader? _getCacheAnnotation(MethodElement method) {
    final annotation = _typeChecker(diopa.CacheControl)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annotation != null) return ConstantReader(annotation);
    return null;
  }
  ...
  ConstantReader? _getMultipartAnnotation(MethodElement method) {
    final annotation = _typeChecker(diopa.MultiPart)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annotation != null) return ConstantReader(annotation);
    return null;
  }
  ...
  ConstantReader? _getFormUrlEncodedAnnotation(MethodElement method) {
    final annotation = _typeChecker(diopa.FormUrlEncoded)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annotation != null) return ConstantReader(annotation);
    return null;
  }
  ...
  ConstantReader? _getResponseTypeAnnotation(MethodElement method) {
    final annotation = _typeChecker(diopa.DioResponseType)
        .firstAnnotationOf(method, throwOnUnresolved: false);
    if (annotation != null) return ConstantReader(annotation);
    return null;
  }
  ...
  Expression _generatePath(MethodElement m, ConstantReader method) {
    final paths = _getAnnotations(m, diopa.Path);
  ...
  Code _generateRequest(MethodElement m, ConstantReader httpMethod) {
    ...
    final cancelToken = _getAnnotation(m, diopa.CancelRequest);
    if (cancelToken != null)
      namedArguments[_cancelToken] = refer(cancelToken.item1.displayName);

    final sendProgress = _getAnnotation(m, diopa.SendProgress);
    if (sendProgress != null)
      namedArguments[_onSendProgress] = refer(sendProgress.item1.displayName);

    final receiveProgress = _getAnnotation(m, diopa.ReceiveProgress);
    ...
    final bool isWrappered =
        _typeChecker(diopa.HttpResponse).isExactlyType(wrapperedReturnType);
    ...
    if (returnType == null || "void" == returnType.toString()) {
      ...
      if (_typeChecker(List).isExactlyType(returnType) ||
        ...
        if (_isBasicType(innerReturnType)) {
          ...
          if (clientAnnotation.parser == diopa.Parser.FlutterCompute) {
            blocks.add(refer('$_resultVar.data')
                .conditionalIsNullIf(
...
''');
  ...
  Expression _parseOptions(
    ...
    final annoOptions = _getAnnotation(m, diopa.DioOptions);
    if (annoOptions == null) {
  ...
  void _generateQueries(
    ...
    final queries = _getAnnotations(m, diopa.Query);
    ...
    final queryMap = _getAnnotations(m, diopa.Queries);
  ...
  void _generateRequestBody(
    ...
    final _noBody = _getMethodAnnotationByType(m, diopa.NoBody);
    if (_noBody != null) {
      blocks
    ...
    var annotation = _getAnnotation(m, diopa.Body);
    ...
    if (_bodyName != null) {
      ...
      if (TypeChecker.fromRuntime(Map).isAssignableFromType(_bodyName.type)) {
        ...
        if (clientAnnotation.parser == diopa.Parser.MapSerializable) {
          ...
          if (toMap == null) {
            ...
            if (_bodyName.type.nullabilitySuffix != NullabilitySuffix.question) {
              blocks.add(refer("$_dataVar.addAll").call([
                refer("${_bodyName.displayName}.toMap()")
              ]).statement);
            } else {
              blocks.add(refer("$_dataVar.addAll").call([
    ...
    final fields = _getAnnotations(m, diopa.Field).map((p, r) {
    ...
    final parts = _getAnnotations(m, diopa.Part);
    if (parts.isNotEmpty) {
  ...
  Map<String, Expression> _generateHeaders(MethodElement m) {
    ...
    final annosInParam = _getAnnotations(m, diopa.Header);
  ...
  void _generateExtra(
    ...
    var data = <DartObject?, DartObject?>{};
    final extras =
        _typeChecker(diopa.Extras).annotationsOf(m, throwOnUnresolved: false);
    final singleExtras =
        _typeChecker(diopa.Extra).annotationsOf(m, throwOnUnresolved: false);

    for (var extra in extras) {
      final c = ConstantReader(extra);
      var m = c.peek('data')?.mapValue;
      if (m != null) {
        data.addAll(m);
      }
    }

    for (var extra in singleExtras) {
      final c = ConstantReader(extra);
      data.putIfAbsent(
          c.peek('name')?.objectValue, () => c.peek('value')?.objectValue);
    }
...
Builder generatorFactoryBuilder(BuilderOptions options) => SharedPartBuilder(
    [DiopaGenerator(DiopaOptions.fromOptions(options))], "diopa");
